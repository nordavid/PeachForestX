...
public class KillPlayerMenu extends PaginatedMenu {
    ...
    public void handleMenu(InventoryClickEvent e) {
        ...
        if (e.getCurrentItem().getType().equals(Material.PLAYER_HEAD)) {
            ...
            PlayerMenuUtility playerMenuUtility = PeachForestX.getPlayerMenuUtility(p);
            playerMenuUtility.setPlayerToKill(Bukkit.getPlayer(UUID.fromString(e.getCurrentItem().getItemMeta().getPersistentDataContainer().get(new NamespacedKey(PeachForestX.getInstance(), "uuid"), PersistentDataType.STRING))));
        ...
        if(players != null && !players.isEmpty()) {
            for(int i = 0; i < getMaxItemsPerPage(); i++) {
                ...
                if (players.get(index) != null){
                    ...
                    playerMeta.getPersistentDataContainer().set(new NamespacedKey(PeachForestX.getInstance(), "uuid"), PersistentDataType.STRING, players.get(index).getUniqueId().toString());
